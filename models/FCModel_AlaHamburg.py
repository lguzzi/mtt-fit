...
class FCModel_Functional(Model):
  ...
  def compile(self,part=None,alpha=1E-5,beta=10.,gamma=1.):
    ...
    #end feature extraction next step is regression NOT linked to classifier
    self.extractedLayersForFollowingStep = Concatenate(axis=-1)([self.layer7,self.layer6])
    self.layer8 = Dense(self.neurons,**self.DENSE_SETUP,name="layer8",trainable=self.train_part1)(self.extractedLayersForFollowingStep)  
    ...
    self.neutrini_layer = Dense(self.n_targets, **self.LAST_SETUP,name="neutrini_layer",trainable=self.train_part1)(self.layer11)    
    ...
    self.layer21 = Dense(self.neurons,**self.DENSE_SETUP,name="layer21",trainable=self.train_part2)(self.extractedLayersForFollowingStep)
    ...
    self.classifier = Dense(3,activation=tf.keras.activations.softmax,name="classifier",trainable=self.train_part2)(self.layer41) 
