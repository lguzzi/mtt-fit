...
from models.FCModel_Functional_LSTM import FCModel_Functional 
import pandas as pd
...
for k in range(len(args.input)):          
    ...
    model_TTsem.load() 
    #model_TTsem.load_weights()
    output = recoMass(model_TTsem)
    #myDF = np.array([output[0],output[1],output[2]])    
    myDF = np.array([np.zeros((len(output[0]),)),output[0],output[1]])  
    if "ggF_BSM_v20" in os.path.basename(args.input[k]).strip('.h5'):
      tautauMass_ggF = output[0]
